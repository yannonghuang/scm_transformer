...
def compute_simple_bom_mask(src_tokens, tgt_tokens, from_make_to_bom=True):
    ...
    if from_make_to_bom:
        ...
        type_match = torch.ones_like(type_equal)
...
def compute_bom_mask(src_tokens, tgt_tokens):
    ...
    T = T_primitive & same_demand & same_location  # [B, T, T]
...
def get_method_attention_bias(src_tokens, tgt_tokens):
    """
...
def compute_simple_method_mask(src_tokens, tgt_tokens):
    ...
    TYPE_METHOD = get_token_type('method')
    TYPE_MAKE = get_token_type('make')
    ...
    is_method = src_tokens['type'] == TYPE_METHOD       # [B, S]
    ...
    ############################################################################################################
    type_match_demand_to_method = (is_make | is_purchase | is_move | is_demand).unsqueeze(2) & is_method.unsqueeze(1)
    mask_bool_demand_to_method = type_match_demand_to_method & material_match & location_match     # [B, T, S]
    ##################
    type_match_method_to_workorder = (is_make | is_purchase | is_move).unsqueeze(2) & is_method.unsqueeze(1)
    mask_bool_method_to_workorder = type_match_method_to_workorder & material_match & location_match & lead_time_match     # [B, T, S]
    ##################
    type_match_method_to_workorder_ms = (is_make | is_purchase | is_move).unsqueeze(2) & is_method.unsqueeze(1)                # [B, T, S]
...
def compute_method_mask(src_tokens, tgt_tokens):
    ...
    workorder_to_workorder = W_primitive & same_demand  # [B, T, T]
...
def SAVE_compute_method_mask(src_tokens, tgt_tokens):
    B, S = src_tokens['type'].shape
    _, T = tgt_tokens['type'].shape
    ...
    type_match_ms = (is_make | is_purchase | is_move).unsqueeze(2) & is_method.unsqueeze(1)                # [B, T, S]
    mask_bool_ms = type_match_ms & material_match & location_match_ms & demand_match     # [B, T, S]
        
    type_match = (is_make | is_purchase | is_move | is_demand).unsqueeze(2) & is_method.unsqueeze(1)
    #mask = type_match & material_match
    # Final boolean mask: True = allowed, False = disallowed
    mask_bool = type_match & material_match & location_match & demand_match & lead_time_match     # [B, T, S]

    # Convert to float mask for additive attention
    # True → 0.0, False → -inf
    mask_method = torch.where(mask_bool, torch.tensor(0.0, device=mask_bool.device),
                                  torch.tensor(float('-inf'), device=mask_bool.device))

    mask_move_source = torch.where(mask_bool_ms, torch.tensor(0.0, device=mask_bool_ms.device),
                                  torch.tensor(float('-inf'), device=mask_bool_ms.device))
    
    return mask_method, mask_move_source
